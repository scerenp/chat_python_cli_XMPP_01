...
class AvatarSetter(sleekxmpp.ClientXMPP):
    ...
    def start(self, event):
        ...
        used_xep84 = False
        try:
            print('Publicando XEP-0084 data avatar')
            self['xep_0084'].publish_avatar(avatar)
            used_xep84 = True
        except XMPPError:
            print('Erro al Publicar el  XEP-0084 avatar')

        try:
            print('Actualizando vCard con avatar')
            self['xep_0153'].set_avatar(avatar=avatar, mtype=avatar_type)
        except XMPPError:
            print('No se pudo actualizar vCard avatar')
        ...
        print('Espere para que la actualizacion de presencia de propague...')
...
if __name__ == '__main__':
    ...
    if opts.filepath is None:
        opts.filepath = input("Imagen del Avatar: ")
